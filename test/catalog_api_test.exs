defmodule CatalogApiTest do
  ...
  describe "cart_remove_item/4" do
    test "returns a message if the response is successful" do
      response_fixture = FixtureHelper.retrieve_json_response("cart_remove_item_success")
      with_mock HTTPoison, get: fn _url -> {:ok, response_fixture} end do
        response = CatalogApi.cart_remove_item(123, 500, 100)
  ...
  describe "cart_order_place/3" do
    ...
    test "returns an error tuple if the cart is not found" do
      body =
        "{\"Fault\": {\"faultcode\": \"Client.APIError\", \"faultstring\": \"Cart not found.\", \"detail\":null}}"
    ...
    test "returns an error tuple if the cart has no shipping address" do
      body =
        "{\"Fault\": {\"faultcode\": \"Client.APIError\", \"faultstring\": \"A shipping address must be added to the cart.\", \"detail\": null}}"
